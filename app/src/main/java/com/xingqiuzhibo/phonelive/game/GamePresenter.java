package com.xingqiuzhibo.phonelive.game;
...
import com.xingqiuzhibo.phonelive.event.LhGameEnterEvent;
import com.xingqiuzhibo.phonelive.game.bean.BankerBean;
...
import com.xingqiuzhibo.phonelive.game.views.AbsGameLhViewHolder;
import com.xingqiuzhibo.phonelive.game.views.AbsGameViewHolder;
import com.xingqiuzhibo.phonelive.game.views.GameEbbViewHolder;
import com.xingqiuzhibo.phonelive.game.views.GameHdViewHolder;
import com.xingqiuzhibo.phonelive.game.views.GameLh_GzViewHolder;
import com.xingqiuzhibo.phonelive.game.views.GameLh_ZbViewHolder;
import com.xingqiuzhibo.phonelive.game.views.GameNzViewHolder;
import com.xingqiuzhibo.phonelive.game.views.GameZjhViewHolder;
import com.xingqiuzhibo.phonelive.game.views.GameZpViewHolder;
...
public class GamePresenter {
    ...
    private static final int GAME_ACTION_LHH = 6;//龙虎和
    ...
    public void setGameParam(GameParam param) {
        ...
        if (!anchor) {
            ...
            GameLhBean gameLhBean = null;
            if (!obj.getString("game_longhu").equals("[]")) {
                gameLhBean = JSON.parseObject(obj.getString("game_longhu"), GameLhBean.class);
            }
            ...
            if (gameLhBean != null) {
                createGameViewHolder(GAME_ACTION_LHH);
                if (mGameViewHolder != null) {
                    mGameViewHolder.setGameID(gameLhBean.getId());
                    mGameViewHolder.setBetTime(gameLhBean.getTime());
                    ((AbsGameLhViewHolder) mGameViewHolder).setEnterGameLhBean(gameLhBean);
                    mGameViewHolder.enterRoomOpenGameWindow();
                    EventBus.getDefault().post(new LhGameEnterEvent());
                }
            }
        }
    }
    ...
    public void startGame(int gameAction) {
        ...
        if (mGameViewHolder instanceof AbsGameLhViewHolder) {
//            if (!mGameViewHolder.isBetStarted() && ((AbsGameLhViewHolder) mGameViewHolder).isCanBalance()) {
                ToastUtil.show(R.string.game_wait_end);
                return;
//            }
        }
        ...
        switch (gameAction) {
            ...
            case GAME_ACTION_EBB:
                ...
                break;
            case GAME_ACTION_LHH:
                HttpUtil.dragonBegin(mGameParam.getStream(), new HttpCallback() {
                    @Override
    ...
    private void createGameViewHolder(int gameAction) {
        ...
        switch (gameAction) {
            ...
            case GAME_ACTION_EBB:
                ...
                break;
            case GAME_ACTION_LHH:
                if (mGameParam.isAnchor()) {
                    gameViewHolder = new GameLh_ZbViewHolder(mGameParam, mGameSoundPool);
                } else {
                    gameViewHolder = new GameLh_GzViewHolder(mGameParam, mGameSoundPool);
                }
                break;
        }
     ...
     * @param obj
     */
    public void onGameLhSocket(JSONObject obj) {
        if (mEnd) {
            return;
        }
        int action = obj.getIntValue("action");
        if (action == SocketGameUtil.GAME_ACTION_LH_OPENVIEW) {
            if (mGameViewHolder != null) {
                mGameViewHolder.removeFromParent();
                mGameViewHolder.release();
                mGameViewHolder = null;
        ...
        if (mGameViewHolder != null && mGameViewHolder instanceof AbsGameLhViewHolder) {
            mGameViewHolder.handleSocket(action, obj);
        }
        if (action == SocketGameUtil.GAME_ACTION_LH_CLOSE) {
            mGameViewHolder = null;
            ((LiveActivity) mContext).setGamePlaying(false);
        }
    }
